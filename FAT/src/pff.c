...
static data FATFS *FatFs;	/* Pointer to the file system object (logical drive) */
...
)
{
	...
	if(buff==0)
	{
		UART_SendStr("malloc error when running f_getfree!\n");
		return 0x0f;
	}
	fs = fatfs;
	if((fs->fs_type)!=FS_FAT32)
	{
		UART_SendStr("Not FAT32!\nPlease format the SD card into FAT32 on your computer!\n");
		Free(buff);
		return 0x0f;
	}
	//* Get number of free clusters
	UART_SendStr("Calculating SD card capacity...\n");
	n = 0;
	clst = fs->n_fatent;
	sect = fs->fatbase;
	i = 0; p = 0;
	do {
		if (!i) {
			res = disk_readp(buff,sect++,0,512);
			if (res != FR_OK) break;
			p = buff;
			i = 512;
		}
