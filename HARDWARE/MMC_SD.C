...
void TEST_SD()
{
	...
	Buffer=Malloc(512);
	if(0==(u32)Buffer)
		UART_SendStr("malloc failed for <Buffer[512]>\n");
	else
	{
		while(1)//检测不到SD卡
		{	
			...
			if(flag)
			{
				UART_PutInf("SD Card Error:",flag);
				Delay_Ms(500);
		...
		UART_SendStr("SD card init OK!!\n");

		flag=0;
		
		for(i=0;i<512;i++)
			Buffer[i]=i;	   	//向数据缓冲区中写入0~255 0~255，共512个字节

		SD_WriteDisk(Buffer,TEST_ADDR,1);
		
		for(i=0;i<512;i++) 		//清空数据缓冲区
			Buffer[i]=0;

		SD_ReadDisk(Buffer,TEST_ADDR,1);
		
		for(i=0;i<512;i++) 
		{
			UART_SendByte(Buffer[i]);
			if(((unsigned char)i)!=Buffer[i])  //对读出的数据进行匹配，如果不吻合则flag=1，否则flag=0
			{
				flag=1;
				break;
			}
		...
		if(flag)
		{
			UART_SendStr("SD card failed!\n"); //数据不吻合
		}
		else 
		{
			UART_SendStr("SD card test ok！\n"); //数据吻合
			sd_size=SD_GetSectorCount();//得到扇区数
			UART_PutInf("SD Card Size(MB):",sd_size/2048);
		}
		Free(Buffer);
	}
}
