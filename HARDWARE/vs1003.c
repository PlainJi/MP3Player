...
#include "uart.h"
//vs的指令表
static const u16 code DataTab[943] = {
...
void VS_LoadPatch(void)
{
	...
	for(i=0;i<sizeof(DataTab);i++)
	{
		switch(i)
		{
			...
			case 798:
			case 927:
				VS_WriteReg(7,DataTab[i]); 
				break;
			case 942:
				VS_WriteReg(10,DataTab[i]); 
				break;
			default:
				VS_WriteReg(6,DataTab[i]); 
		}		
	}
	Delay(500);	
}
...
{
	u8 i;
	VS_WriteReg(VS_WRAMADDR,0x1804);
	for(i=0;i<14;i++)
	{
		*p++=VS_ReadReg(VS_WRAM);
	}
}
#endif

//0-254,255 analog powerdown mode
//0-31,开机时设置为20(0x5E5E)
...
{ 
	if(volume>31)
		...
		volume=1;
	volume = 254-(volume<<3);
	VS_WriteReg(VS_VOL,((volume<<8)|volume));
}

void VS_Init(void)
{
	P1M0|=0XE0;				//1110 0000 SPI接口推挽输出
	spi_init(16);
	VS_RST=1;
	Delay(100);
	VS_RST=0;				//硬件复位
	...
	VS_WriteReg(VS_MODE  ,0x0804);  /* 软件复位，向0号寄存器写入0x0804   SM_SDINEW为1   SM_RESET为1 */
	VS_WriteReg(VS_CLOCKF,0x9800);  /* 时钟设置，向3号寄存器写入0x9800   SC_MULT  为4   SC_ADD  为3   SC_FREQ为0 */
	VS_WriteReg(VS_VOL   ,0x5e5e);  /* 音量设置，左右声道均中等音量,5e,vol=20 */
	VS_WriteReg(VS_BASS	 ,0x7aaa);	/* 音效设置 0111 1010 1010 1010,10KHz以上提升10.5dB,100Hz以下提升10dB*/
	VS_DCS=0;	     			    /* 打开数据片选 */
	...
	VS_WriteByte(0);
	VS_DCS=1;	    			    /* 关闭数据片选 */
	VS_SinTest(1000);VS_SinTest(1000);
	UART_SendStr("VS1003 Init...		OK\n");
}
...
void VS_SendDat(u8 dat) 
{
	...
	VS_WriteByte(dat);	//通过SPI向VS1003写入一个字节的音频数据
}

//当DREQ为高时，至少可以送32字节的数据
...
{
	return VS_ReadReg(VS_DECODE_TIME);
}

void VS1003B_SetDecodeTime0()
{
	VS_WriteReg(VS_DECODE_TIME,0x0000);
}

//比特率预定值,阶层III
code u16 bitrate[2][16]=
{ 
...
u16 VS_Get_HeadInfo(void)
{
	...
	u16 HEAD1;
 	...
 	HEAD0=VS_ReadReg(VS_HDAT0); 
    HEAD1=VS_ReadReg(VS_HDAT1);
...
void VS_SetBass(u8 treble,u8 tfreq,u8 bass,u8 bfreq)
{
	...
	VS_WriteReg(VS_BASS,bass_set);	//BASS
}
